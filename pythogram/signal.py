...
class Signal(object):
  ...
  def signal(self):
    ...
    # securing the filters against under/overflow (e.g. after changing the
    # sample rate)
    try:
    ...
    # b, a = iirfilter(5, Wn=[low, high], btype='bandpass', ftype='butter')
    b, a = butter(5, Wn=[low, high], btype='band')
    
    signal = filtfilt(b, a, self._signal, method='gust', irlen=int(nyq)).astype(
      np.float32)
    # signal = lfilter(b, a, self._signal).astype(np.float32)
  ...
  def __add__(self, other):
    ...
    if self.sample_rate != other.sample_rate:
      other_signal = resample(other._signal,
                              int(other.length * self.sample_rate))
    else:
  ...
  def low_cutoff(self, value):
    ...
    if value < self.sample_rate * 0.5 * 0.00045:
      raise ValueError('low cutoff must at least be %f hz' % (
      self.sample_rate * 0.5 * 0.00045))
  ...
  def high_cutoff(self, value):
    ...
    if value <= self.low_cutoff:
      raise ValueError(
        "high cutoff may not be equal or lower than the low cutoff")
    
    if value > float(self.sample_rate * 0.5) - (self.sample_rate * 0.5 * 0.05):
      raise ValueError("high cutoff may not exceed %f" % (
      float(self.sample_rate * 0.5) - (self.sample_rate * 0.5 * 0.045)))
